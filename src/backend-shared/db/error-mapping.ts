...
export function mapMysqlError(err: unknown): DatabaseError {
	...
	if (errno === 1451 || errno === 1452 || /foreign key constraint/i.test(message) || /cannot delete or update a parent row/i.test(message) || /cannot add or update a child row/i.test(message)) {
		return new ConstraintError("CONSTRAINT_FK", message, { cause: err });
	}
